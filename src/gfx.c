...
void drawText(const char* str, tex* target, const font* f, int x, int y, int sz, clr c, uint32_t hashColor)
{
	int tmpX = x;
	uint32_t tmpChr = 0;
	ssize_t unitCnt = 0;
	...
	for (unsigned i = 0; i < length; )
	{
		...
		case '|':
		{
			if (clrGetColor(textClr) == 0xFF00FFFF)
				textClr = originalClr;  // Return to original color instead of white
			else
		...
		case '&':
		{
			if (clrGetColor(textClr) == 0xFF009dff)
				textClr = originalClr;  // Return to original color instead of white
			else
		...
		case '~':
		{
			if (clrGetColor(textClr) == 0xFFFFFFFF)
				textClr = originalClr;  // Return to original color instead of white
			else
		...
		case '#':
		{
			if (clrGetColor(textClr) == hashColor)
				textClr = originalClr;
			else
				textClr = clrCreateU32(hashColor);  // Use the parameter instead of hardcoded value
			continue;
		}
		}
...
void drawTextWrap(const char* str, tex* target, const font* f, int x, int y, int sz, clr c, int maxWidth, uint32_t hashColor)
{
	char wordBuf[128];
	size_t nextbreak = 0;
